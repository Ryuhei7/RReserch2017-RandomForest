...
# UnderSamplingを施す
rus = RandomUnderSampler(ratio={1.0:int(positive_count_train[2.0]), 2.0:int(positive_count_train[2.0]), 3.0:int(positive_count_train[2.0]*5)}, random_state=0)
rus_data, rus_label = rus.fit_sample(train_data, train_label)

# SMOTEを施す
smote = SMOTE(ratio={1.0:int(positive_count_train[2.0]*2), 2.0:int(positive_count_train[2.0]*2), 3.0:int(positive_count_train[2.0]*5)}, random_state=0)

# SMOTEを学習データに反映
train_data_resampled, train_label_resampled = smote.fit_sample(rus_data, rus_label)

# 学習する
clf = OneVsOneClassifier(RandomForestClassifier(random_state=0, n_estimators=500))
